...
layout(set = 2, binding = 0) uniform UBODirectionalLight {
   DirectionalLight light;
} directionalLight;

layout(set = 2, binding = 1) uniform UBOPointLights {
   PointLight light[1];
} pointLights;
...
float ShadowCalculation(const vec4 fragPosLightSpace, const vec4 normal, const vec4 lightDir, const sampler2D map)
{
...
vec4 CalculateDirectionalLight(DirectionalLight light, vec4 viewDir, vec4 normal, vec3 diffuseColor, vec3 specularColor) {
   ...
   float shadow = ShadowCalculation(inFragPosLightSpace, normal, lightDir, shadowMap);
...
vec4 CalculatePointLight(PointLight light, vec4 viewDir, vec4 normal, vec3 diffuseColor, vec3 specularColor) {
   ...
   return vec4(
      ((diffuseColor * diffuse) + (specular * vec3(specularColor.r))) * light.color * attenuation, // specularity in specularmap red channel
      1.0
   );
}
