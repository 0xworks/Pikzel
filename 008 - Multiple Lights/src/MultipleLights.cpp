...
public:
   ...
   , m_Input {GetWindow()}
   {
      ...
      m_Camera.Projection = Pikzel::RenderCore::ClipSpace() * glm::perspective(m_Camera.FoVRadians, static_cast<float>(GetWindow().GetWidth()) / static_cast<float>(GetWindow().GetHeight()), 0.1f, 100.0f);
   }
