...
namespace Pikzel {
   ...
   void VulkanGraphicsContext::SubmitSingleTimeCommands(const std::function<void(vk::CommandBuffer)>& action) {
      std::vector<vk::CommandBuffer> commandBuffers = m_Device->GetVkDevice().allocateCommandBuffers({
         m_CommandPool                    /*commandPool*/,
         vk::CommandBufferLevel::ePrimary /*level*/,
      ...
      m_Device->GetGraphicsQueue().submit(si, nullptr);
      m_Device->GetGraphicsQueue().waitIdle();
      m_Device->GetVkDevice().freeCommandBuffers(m_CommandPool, commandBuffers);
   }
